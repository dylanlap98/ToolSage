...
These dimensions need to be independent. Their divergence is the signal `sage.improve()` uses to identify where a manifest needs improvement: if an agent ignores the manifest and still gets great results, that's evidence the manifest guidance is too restrictive or wrong. If an agent follows the manifest perfectly and still gets poor results, the manifest is missing something.
...
`sage.improve()` looks for patterns across many scored calls, specifically cases where `output_quality` and `manifest_adherence` diverge consistently across a usage category. That divergence is weak signal in any single run but becomes strong signal across many calls. Correlated scores would make the divergence invisible entirely, defeating the purpose of having two dimensions. Separating the calls is a prerequisite for the self-improvement loop to work.
